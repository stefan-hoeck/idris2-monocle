...
||| relevance. We need them in order to compose `Fold`s with other
||| optics (see module `Monocle.Compose`).
|||
||| `Fold`s are mainly useful because almost all other optics can be
||| converted to `Fold`s (with the exception of `Setter`s).
public export
...
||| Sequential composition of `Fold`s.
public export %inline
...
||| Interface for converting other optics to `Fold`s.
public export
